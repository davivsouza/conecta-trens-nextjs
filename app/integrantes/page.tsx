  ...
  return (
    <div className="flex flex-col min-h-screen">
        ...
        <div className="container mx-auto flex items-center">
          <Link href="/" className="flex items-center">
            <Image 
              ...
              className="mr-2"
            />
            <strong className="text-xl font-bold" aria-label="Nome da empresa">ConectaTrens</strong>
          </Link>
        </div>
      </header>
      ...
      <main className="flex-grow container mx-auto px-4 py-8" role="main">
        ...
        >
          {teamMembers.map((member, index) => (
            <li key={index} className="bg-white rounded-lg shadow-md overflow-hidden flex flex-col items-center p-4">
              ...
              <p className="font-medium text-center mb-2">{member.name} - RM: {member.rm}</p>
      ...
      <footer className="bg-gray-800 text-white p-4 text-center" role="contentinfo">
        <p>&copy; {new Date().getFullYear()} ConectaTrens. Todos os direitos reservados.</p>
      </footer>
    </div>
  );
}